...
class TestPluginLoading:
    ...
    def test_merge_plugin_into_request_empty_plugin(self, conversation_service):
        ...
        result, hook_config = conversation_service._merge_plugin_into_request(
            request, empty_plugin
        )

        # Request should be unchanged - agent_context stays None
        assert result.agent.agent_context is None
        # No hooks from empty plugin
    ...
    def test_load_and_merge_plugin_success(
        ...
        result, hook_config = conversation_service._load_and_merge_plugin(request)
    ...
    def test_merge_plugin_into_request_at_skill_limit(
        ...
        # Should not raise
        result, _ = conversation_service._merge_plugin_into_request(request, plugin)
        assert len(result.agent.agent_context.skills) == MAX_PLUGIN_SKILLS

    def test_merge_plugin_with_hooks_returns_hook_config(
        self, conversation_service, caplog
    ):
        """Test that plugin hooks are returned separately from the request."""
        import logging
        ...
        with caplog.at_level(logging.INFO):
            updated_request, hook_config = (
                conversation_service._merge_plugin_into_request(
                    request, plugin_with_hooks
                )
            )

        # Verify hooks were returned
        assert hook_config is not None
        assert "PreToolUse" in hook_config.hooks
        assert len(hook_config.hooks["PreToolUse"]) == 1
        assert hook_config.hooks["PreToolUse"][0].matcher == "terminal"
        # Verify info was logged about hook loading
        assert "Loaded hooks" in caplog.text

    def test_merge_plugin_with_only_hooks_returns_hook_config(
        self, conversation_service, caplog
    ):
        """Test plugin with only hooks returns hook_config separately."""
        import logging
        ...
        # Hook config should be returned separately
        assert hook_config is not None
        assert "PostToolUse" in hook_config.hooks
        # Agent should remain unchanged (no skills/mcp)
        assert updated_request.agent.agent_context is None


class TestPluginMcpConfig:
    """Test cases for plugin MCP config and skill merging."""
